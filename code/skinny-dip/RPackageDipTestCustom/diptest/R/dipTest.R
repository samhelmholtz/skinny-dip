...
dip.test <- function(x, simulate.p.value = FALSE, B = 2000)
{
    ...
    dipOutput <- capture.output(dipFullResult <- dip(x, debug=TRUE, full.result=TRUE))
    D <- dipFullResult$dip
    ...
    structure(list(statistic = c(D = D), dipFullResult=dipFullResult, dipOutput=dipOutput, p.value = P, nobs = n,
