...
#define l_lcm lo_hi[3]
 ...
 LOOP_Start:
  ...
  int j_best, j_l = -1, j_u = -1, lcm_modalTriangle_i1 = -1,lcm_modalTriangle_i3 = -1, gcm_modalTriangle_i1 = -1,gcm_modalTriangle_i3 = -1;

  /* The DIP for the greatest convex minorant. */
  dip_l = 0.;
  for (j = ig; j < l_gcm; ++j) {
    double max_t = 1.;
  ...
  /* The DIP for the least concave majorant. */
  dip_u = 0.;
  for (j = ih; j < l_lcm; ++j) {
    double max_t = 1.;
  ...
  if (*dip < dipnew) {
    *dip = dipnew;
    ...
    if(*debug){
      if(dip_u > dip_l) {
      	dipnew = dip_u; j_best = j_u;
	Rprintf(" -> new larger dip %g (j_best = %d) gcm-centred triple (%d,%d,%d)\n", dipnew, j_best,lcm_modalTriangle_i1,j_best,lcm_modalTriangle_i3);
      } else {
      	dipnew = dip_l; j_best = j_l;
	Rprintf(" -> new larger dip %g (j_best = %d) lcm-centred triple (%d,%d,%d)\n", dipnew, j_best,gcm_modalTriangle_i1,j_best,gcm_modalTriangle_i3);
      }
    }      
  }
